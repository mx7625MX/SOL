...
包含两个专用的Solana pump.fun交易机器人：
1. **狙击机器人** (`sniper_bot.py`) - 监控并自动购买新发布的代币
2. **利润监控机器人** (`pump_profit_bot.py`) - 监控持仓并自动卖出获利
...
### 主要功能：
- ✅ 实时监测自己的钱包首次买入后所有新代币（自动识别新买的Token）
- ✅ 对你的新买代币，持续监控钱包持仓利润
- ✅ 当利润达到预设百分比（如30%），立即自动卖出
- ✅ 检测到其它钱包买入且买入量大于指定数额（如1 SOL），立即自动卖出
- ✅ 卖出比例可配（10%-100%）
- ✅ 卖出延迟严格≤2000毫秒，优先速度，防止被狙（sniper bot）抢先卖出
- ✅ 所有参数（利润百分比、触发金额、卖出比例）都可配置

**详细文档**: 请查看 [PROFIT_BOT_README.md](./PROFIT_BOT_README.md)
...
**当前实现状态**: 本项目提供了完整的框架和监控系统，但**pump.fun的实际交易执行功能尚未完全实现**。

要使机器人能够真正执行交易，需要：
1. 对pump.fun合约进行逆向工程以获取准确的交易指令格式
2. 实现bonding curve价格计算
3. 构建完整的买卖交易指令
...
- ✅ 配置和日志系统
- ✅ 交易框架和错误处理
- ⚠️ 交易执行功能（框架已搭建，需要实现具体逻辑）

**适用于**: 学习Solana开发、理解pump.fun机制、作为开发基础进行二次开发。
